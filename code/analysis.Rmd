...
abstract: "This analysis will try to answer how you can price a laptop based on its specifications and how you can try to find good deals among them. It uses 1300 data points to build a linear regression model on price using different properties like company that manufactured it, cpu model, RAM size, etc."
---
...
- not representative, price can change based on release date
- quality? -> no classical measurement error but can be still erroneous inputs (don't know the source, kaggle)
- this is administrative data 
- question -> does the price only depend on the specs? no, design, build quality, materials, marketing, etc. -> but otherwise it captures pretty much everything

- everything will be a variable, except if it needs to be removed

## Data cleaning

- cleaning the strings (ram, weight, etc.) 
- data mining screen -> is it IPS? is it touchscreen? -> creating new variables.function that extracts model, manufacturer, etc.
- dummy coding and baselines:
  - op_sys -> win10
  - type_name -> Notebook
  - screen_category -> screen_mid
...
We can see that the log transformed version of prices is the closest to distribution.

Detailed plots on the pattern of association between log-price and covariates can be found in the [appendix](#appendix).

```{r include=FALSE}
summtable <- summary(laptop$price_euros)
...
knitr::kable(summtable, format = "html", caption = "Summary statistics for price")
...
```{r include=FALSE}
reg3 <- lm(step_formula, data = laptop, singular.ok = F)
...
> todo: compare R2 of both models, create standardized Betas, try to interpret final results
...
Now that we are sure our model met all our assumptions we can analyse the residuals to find the most valuable deals. I limited the results to be under $2000 because above that it might be that our model is less accurate and there are other factors in pricing premium products as well. Based on this, we get the following results.
