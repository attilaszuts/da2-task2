...
For the baseline model I am going to use a simple regression of log price on the manufacturer company. I decided to use ln prices, as it was skewed to the left, having a long right tail and this transformation made it normal. I will be careful at the interpretation to use the correct one.
...
To find out which coefficients provide the best fit for my data I ran a simulation to find the best possible fit using AIC.
...
There are in total `r nrow(large_resid)` outliers. This in itself is not necessarily a bad thing, it might be that they are very good deals. However it is worth taking a look at them to see if they exert undue influence on the model which could in turn distort the results. Investigating the cook's distance of the outliers we can find that there is `r cookd_over_thresh` observation above 1. 

## Multicollinearity

Upon inspecting the first model (reg3), I found that there is multicollinearity among the variables. So I had to investigate which variables cause this, and I found that there was multicollinearity among the operating system, resolution gpu type and cpu model. I experimented to see if there are some combination of these variables that can be included in the model, without multicollinearity but only the cpu model could be used. So due to this, I had to go back and exclude the aforementioned variables. But even with this modification, there still may be some collinearity among my variables, as the average VIF is above 1.

Multicollinearity is an issue, since it limits the $R^2$ of the model and increases the SE of the $\beta$ coefficients. This makes it more difficult to interpret the results. 
...
Now that we are sure our model met all our assumptions we can analyse the residuals to find the most valuable deals.
